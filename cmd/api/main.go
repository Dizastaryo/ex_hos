...
func main() {
	...
	accessService := service.NewAccessService(accessRepo, scannerRepo, logger)
	accessHandler := handler.NewAccessHandler(accessService, logger)
	chatHandler := handler.NewChatHandler(chatService, accessService, logger)
	...
	roomHandler := handler.NewRoomHandler(roomService, accessService, validate, logger)
	...
	// Scanner — BLE resolve (returns real user accounts)
	...
	// Access system: QR-based mutual messaging permission
	access := api.Group("/access", middleware.Auth(jwtManager, sessionStore, userRepo))
	access.Post("/qr", accessHandler.GenerateQR)
	access.Post("/scan", accessHandler.ScanQR)
